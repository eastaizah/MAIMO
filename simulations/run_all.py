...
``comparisons.json`` Welch t-tests against MAIMO with Holm-Bonferroni
...
from maimo.stats import holm_bonferroni, summarise, t_critical, welch_t_test
...
def main() -> int:
    ...
    # ---- Welch tests against MAIMO, Holm-Bonferroni per family --------
    ...
    for family, members in (
        ...
        for m in SCALAR_METRICS:
            ...
            stat = {}
            for pid in members:
                t, p = welch_t_test(np.asarray(ref[m], dtype=float),
                                    np.asarray(results[pid][m], dtype=float))
                praw[pid] = p
                stat[pid] = t
            corrected = holm_bonferroni(praw)
            for pid in members:
                corrected[pid]["t"] = stat[pid]
            fam[m] = corrected
