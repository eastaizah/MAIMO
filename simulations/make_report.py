...
def prose(r: Report, conv: dict, car: Dict[str, Summary]) -> List[str]:
    ...
    A("")
    A(f"Table 7 compares MAIMO against the nine baselines B1-B9 on the full metric set. The comparison is paired: every scheme is driven by the same traffic traces and the same channel realisations, and the learned controllers are trained on a disjoint traffic window and then frozen, so all reported numbers are out of sample. p-values are from the paired two-sided Student t-test on the 20 per-seed differences and are corrected with Holm-Bonferroni across the family of nine comparisons.")
    A("")
    ...
    A("")
    A("This is an empirical convergence profile on this environment and this reward, not a guarantee. PPO's clipped surrogate objective [28] does not come with a monotone improvement guarantee outside the trust-region assumptions of TRPO [39], the environment here is non-stationary by construction, and the policy is a function approximator, so none of the conditions under which monotone improvement can be proved are met. The theoretical statements about this controller are set out in Section 2.5, and no claim of guaranteed convergence or of a sample-complexity bound is made on the basis of these curves.")
    A("")
...
def build(r: Report) -> str:
    ...
    A("")
    A(f"Mean of {r.n} seeds ± 95 % confidence half-width (Student's t, {r.n - 1} d.o.f.). p-values from the paired two-sided Student t-test on the 20 per-seed differences, corrected with Holm-Bonferroni across the twelve comparisons in this table.")
    A("")
    L += table6(r)
    A("")
    ...
    A("")
    A(f"Each row removes exactly one component from the full system; all other parameters, traffic traces and channel realisations are identical to A0. Mean of {r.n} seeds ± 95 % confidence half-width, paired two-sided Student t-test against A0 corrected with Holm-Bonferroni across the family of five comparisons.")
    A("")
    L += table8(r)
    A("")
