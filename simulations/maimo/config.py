...
class Config:
    ...
    horizon_slots: int = 36_000      # 3.6e4 one-second slots = 10 h
    warmup_slots: int = 5_000        # discarded before statistics are collected
    n_seeds: int = 20                # seeds 1..20
    t_crit_19df: float = 2.093       # t_{0.975,19}
    # These four values are the protocol reported in the paper, so that
    # ``python run_all.py`` with no arguments reproduces the published tables.
    # ``long()`` below returns the ten-times-longer protocol for anyone who
    # wants to check that the conclusions are not an artefact of the horizon.
    ...
    pred_epochs: int = 8             # as reported; see long()
    pred_batch: int = 256
    pred_patience: int = 5           # early stopping on validation MSE
    pred_train_intervals: int = 2000    # 20 000 slots of the training trace
    ...
    ppo_updates: int = 40            # PPO updates per seed, as reported
    ...
    dqn_steps: int = 5000            # matched to the PPO interaction budget
    ...
    lyapunov_v: float = 1.2
    """B7 drift-plus-penalty control knob.  The comparator is evaluated at
    this single operating point; no sweep over it is performed anywhere in
    this package, and the paper reports B7 accordingly."""
...
def long(cfg: Config = DEFAULT) -> Config:
    """The ten-times-longer protocol: 3.6e5 slots (100 h) after a 1e4 warm-up.

    The published tables use :data:`DEFAULT`.  This configuration exists so
    that a reader can check the conclusions against a longer horizon and a
    larger training budget; it costs roughly an order of magnitude more
    wall-clock time and is not what the paper reports.
    """
    return cfg.replace(horizon_slots=360_000, warmup_slots=10_000,
                       ppo_updates=150, dqn_steps=19_200,
                       pred_epochs=30, pred_train_intervals=6_000)
